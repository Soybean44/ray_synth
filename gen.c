...
void genTable(char* tableName, FILE *fd, short (*nextSample)(float)) {
  float phase = 0, phase_inc = 1.0f / (tableSize - 1); 
  fprintf(fd, "static const short %s[TABLE_SIZE] = {", tableName);
...
short sineSample(float phase) {
  return (short)(32000.0f * sinf(PI * phase));
}
short sawSample(float phase) {
  return (short)(phase * 32000.0f);
}
